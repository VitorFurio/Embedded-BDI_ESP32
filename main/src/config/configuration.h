 ...
 * +fire : true <- print_fire; .broadcast(tell,sensor2).
 * -fire : true <- print_not_fire; .broadcast(untell,sensor2).
 * 
 * // Reação a mensagens
 * +sensor1 <- +alert.
 * +sensor2 <- +alert.
...
public:
  AgentSettings()
  {
    ...
    inst_1_1.add_arg(list.searchByName("sensor2")->prop);
    ...
    inst_1_2.add_arg(list.searchByName("sensor2")->prop);
